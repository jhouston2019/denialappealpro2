/**
 * extract-denial.js — Free preview extraction from denial letter PDF or pasted text.
...
function normalizeCarc(c) {
  const digits = String(c).replace(/\D/g, "");
  if (!digits) return "";
  return String(parseInt(digits, 10));
}

function normalizeRarc(c) {
...
function applyAliases(data) {
  ...
  if ((!d.icd10_codes || !d.icd10_codes.length) && d.icd_codes?.length) {
...
function postProcess(data) {
  ...
  return {
    ...
    cpt_codes: dedupe(Array.isArray(raw.cpt_codes) ? raw.cpt_codes.map(String) : []),
    icd10_codes: dedupe(
...
function toApiResponse(extracted, rawText) {
  const fc = {
    patientName: fieldConfidence("patient_name", extracted.patient_name, rawText),
    providerName: fieldConfidence("provider_name", extracted.provider_name, rawText),
    providerNpi: fieldConfidence("provider_npi", extracted.provider_npi, rawText),
    payerName: fieldConfidence("payer_name", extracted.payer_name, rawText),
    claimNumber: fieldConfidence("claim_number", extracted.claim_number, rawText),
    ...
    billedAmount: fieldConfidence("billed_amount", extracted.billed_amount, rawText),
...
exports.handler = async (event) => {
  ...
  try {
    ...
    if (contentType.includes("multipart/form-data")) {
      ...
      const filePart = parseMultipart(buffer, contentType);
      ...
      rawText = await extractTextFromPDF(filePart.buffer);
    } else {
      let body = {};
      try {
